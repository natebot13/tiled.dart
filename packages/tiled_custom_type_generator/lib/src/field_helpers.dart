...
const _tiledKeyChecker = TypeChecker.fromRuntime(TiledCustomTypeMember);

bool hasTiledKeyAnnotation(FieldElement element) =>
    _tiledKeyAnnotation(element) != null;

DartObject? _tiledKeyAnnotation(FieldElement element) =>
    _tiledKeyChecker.firstAnnotationOf(element) ??
    (element.getter == null
        ? null
        : _tiledKeyChecker.firstAnnotationOf(element.getter!));
...
class _FieldSet implements Comparable<_FieldSet> {
  ...
  factory _FieldSet(FieldElement? classField, FieldElement? superField) {
    ...
    final fieldHasJsonKey =
        fields.firstWhere(hasTiledKeyAnnotation, orElse: () => fields.first);
